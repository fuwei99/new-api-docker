...
const EditChannelModal = (props) => {
  ...
  const handleAddPrefix = () => {
    const channelName = (formApiRef.current?.getValue('name') || inputs.name || '').trim();
    const currentModels = formApiRef.current?.getValue('models') || inputs.models || [];
    ...
    const prefixedModels = currentModels.map((model) => {
      const trimmedModel = (model || '').trim();
      if (trimmedModel.startsWith(channelName + '/')) {
        return trimmedModel;
      }
      return `${channelName}/${trimmedModel}`;
    });

    const mapping = {};
    prefixedModels.forEach((prefixedModel, index) => {
      const originalModel = currentModels[index].replace(`${channelName}/`, '');
      mapping[prefixedModel] = originalModel;
    });

    const modelMappingJson = JSON.stringify(mapping, null, 2);

    if (formApiRef.current) {
      formApiRef.current.setValue('models', prefixedModels);
      formApiRef.current.setValue('model_mapping', modelMappingJson);
    }
